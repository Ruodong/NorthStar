...
- **EC-9** — Container status bubble-up must collapse all child change signals to "Change", never "New" or "Sunset". An application with a standard CMDB A-id already exists; it cannot become "brand new" just because the architect painted one small sub-module red. Before the fix, the bubble-up in `drawio_parser.py` ran a priority loop `("New", "Change", "Sunset")` and picked the strongest signal — so `A000038 ADM Support` in `adm 应用架构` showed `application_status='New'` in the admin Extracted tab because its `automatic expense allocation` sub-module was `#f8cecc` (New). Fix: any `{New, Change, Sunset} ∩ children_statuses` non-empty → `container.application_status = 'Change'`. The rule is applied regardless of the container's own fill colour (the user's explicit ruling: "不管应用是什么颜色,应用都是 change"). Keep / 3rd-Party fallback still runs only when the container's own status is Unknown, to avoid overwriting intentional container colours. Regression tests: `test_container_status_child_new_becomes_change` and `test_container_status_all_keep_children_stays_keep` in `test_confluence_drawio_extract.py`.
